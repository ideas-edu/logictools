...
function TwoWayExerciseSolver() {
    ...
    this.solve = function (exercise, onExerciseSolved, onErrorSolvingExercise) {
        var state,
			currentStep = exercise.steps.getCurrentStep(),
            currentStrategy,
            currentLocation,
            currentFormula,

            onError = onErrorSolvingExercise,
            ...
            onSuccess = function (data) {
                ...
                var proofSteps = new TwoWayStepCollection(new TwoWayStep(exercise.equation.getText(), null));
	 ...
	 */
		...
		var result = data.result;
                var nextStep = new TwoWayStep(result[3][2], result[0]);
    ...
    this.getHelpForNextStep = function (exercise, onHelpForNextStepFound, onErrorGettingHelpForNextStep) {
        var currentStep = exercise.steps.getCurrentStep(),
            ...
            currentFormula,
            state,
            onError = onErrorGettingHelpForNextStep,
            onSuccess = function (data) {
                if (data === null || data.error !== null || data.result === null) {
                    onErrorGettingHelpForNextStep();
                } else {
                    var proofSteps = new TwoWayStepCollection(new TwoWayStep(currentStep.equation.getText(), null));
