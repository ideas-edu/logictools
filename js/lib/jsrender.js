...
(function (global, jQuery, undefined) {
  ...
  const versionNumber = 'v1.0pre'

  let $; let rTag; let rTmplString; let $extend

  //    compiledTmplsCache = {},
  let delimOpenChar0 = '{'; let delimOpenChar1 = '{'; let delimCloseChar0 = '}'; let delimCloseChar1 = '}'; const deferChar = '!'
  const $viewsSub = {}
  const FALSE = false; const TRUE = true

  const rPath = /^(?:null|true|false|\d[\d.]*|([\w$]+|~([\w$]+)|#(view|([\w$]+))?)([\w$.]*?)(?:[.[]([\w$]+)\]?)?|(['"]).*\8)$/g

  //                               nil   object   helper    view  viewProperty pathTokens   leafToken     string

  const rParams = /(\()(?=|\s*\()|(?:([([])\s*)?(?:([#~]?[\w$.]+)?\s*((\+\+|--)|\+|-|&&|\|\||===|!==|==|!=|<=|>=|[<>%*!:?\/]|(=))\s*|([#~]?[\w$.]+)([([])?)|(,\s*)|(\(?)\\?(?:(')|("))|(?:\s*([)\]])([([]?))|(\s+)/g

  //          lftPrn        lftPrn2                path    operator err                                                eq         path2       prn    comma   lftPrn2   apos quot        rtPrn   prn2   space
  // (left paren? followed by (path? followed by operator) or (path followed by paren?)) or comma or apos or quot or right paren or space

  const rNewLine = /\r?\n/g
  const rUnescapeQuotes = /\\(['"])/g
  const rEscapeQuotes = /\\?(['"])/g
  const rBuildHash = /\x08(~)?([^\x08]+)\x08/g

  let autoTmplName = 0
  const escapeMapForHtml = {
    '&': '&amp;',
    '<': '&lt;',
    '>': '&gt;'
  }
  const tmplAttr = 'data-jsv-tmpl'
  const fnDeclStr = 'var j=j||' + (jQuery ? 'jQuery.' : 'js') + 'views,'
  const htmlSpecialChar = /[\x00"&'<>]/g
  ...
  function getHelper (helper) {
    ...
    const view = this
    const tmplHelpers = view.tmpl.helpers || {}
  ...
  function convert (converter, view, self, text) {
    ...
    const linkContext = !self.markup && self || undefined
    let tmplConverter = view.tmpl.converters
  ...
  function renderTag (tag, parentView, self, content, tagInstance) {
    ...
    let ret
    const linkCtx = !self.markup && self // self is either a template object (if rendering a tag) or a linkCtx object (if linking using a link tag)
    const parentTmpl = linkCtx ? linkCtx.view.tmpl : self
    const tmplTags = parentTmpl.tags
    const nestedTemplates = parentTmpl.templates
    const props = tagInstance.props = tagInstance.props || {}
    let tmpl = props.tmpl
    const args = arguments.length > 5 ? slice.call(arguments, 5) : []
    const tagObject = tmplTags && tmplTags[tag] || $viewsTags[tag]
  ...
  function View (context, path, parentView, data, template, key, onRender, isArray) {
    ...
    const self = {
      ...
      renderLink: function (index) {
        const linkTmpl = this.tmpl.tmpls[index]
  ...
  function renderContent (data, context, parentView, key, isLayout, path, onRender) {
    ...
    let i; let l; let dataItem; let newView; let itemResult; let parentContext; let tmpl; let props; let swapContent; let mergedCtx; let dynCtx; let hasContext
    const self = this
    let result = ''
  ...
  function tmplFn (markup, tmpl, bind) {
    ...
    // result,
    const allowCode = tmpl && tmpl.allowCode
    const astTop = []
    let loc = 0
    const stack = []
    let content = astTop
  ...
  function buildCode (ast, tmpl) {
    ...
    // Used for compiling templates, and also by JsViews to build functions for data link expressions
    let node; let i; let l; let code; let hasTag; let hasEncoder; let getsValue; let hasConverter; let hasViewPath; let tag; let converter; let params; let hash; let nestedTmpl; let allowCode; let content; let attr; let quot
    const tmplOptions = tmpl ? {
      allowCode: allowCode = tmpl.allowCode,
      debug: tmpl.debug
    } : {}
    const nested = tmpl && tmpl.tmpls
  ...
  function parseParams (params, bind, defer) {
    ...
    const fnCall = {}
    let parenDepth = 0
    let quoted = FALSE // boolean for string content in double quotes
    let aposed = FALSE // or in single quotes
    ...
    function parseTokens (all, lftPrn0, lftPrn, path, operator, err, eq, path2, prn, comma, lftPrn2, apos, quot, rtPrn, prn2, space) {
      ...
      prn = prn || prn2 || ''
      operator = operator || ''
      const bindParam = (bind || defer) && prn !== '('
