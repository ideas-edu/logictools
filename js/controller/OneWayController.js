/*global document, OneWayController, OneWayStep, OneWayExercise, OneWayExerciseSolver, OneWayExerciseValidator, SyntaxValidator, jQuery, LogEXSession, $, Resources, KeyBindings, Rules, setTimeout, OneWayExerciseGenerator, formula, window */
(function ($) {
    "use strict";

    // we can now rely on $ within the safety of our "bodyguard" function
    $(document).ready(function () {
        var controller = new OneWayController();		
...
function OneWayController() {
    ...
    */
    ...
    this.useExercise = function (exnr) {
        var ruleJustification = $(SWITCH_RULE).bootstrapSwitch('status'),
            stepValidation = $(SWITCH_VALIDATION).bootstrapSwitch('status');
    ...
    this.createExercise = function () {
		var exerciseMethod = Resources.getExerciseMethod(self.exerciseType),
			ruleJustification = $(SWITCH_RULE).bootstrapSwitch('status'), // true || false
    ...
    */
    ...
    this.onHelpForNextStepFound = function (nextOneWayStep) {
        var language = LogEXSession.getLanguage(),
			...
			state = [self.exercise.type, step.strategyStatus, step.formula, ""];
        $("#formula").popover('destroy');;	
        ...
        $('#toggle-hint1').on('click', function () {
            ...
            var newFormula = nextOneWayStep.formula;
            formula.popover('destroy');
    ...
    this.validateStep = function () {		
        ...
        var changed = $("#formulaoriginal").val() !== $("#formula").val();
    ...
    this.checkIfReady = function () {		
		var step  = this.exercise.getCurrentStep();
		var state = [this.exercise.type, step.strategyStatus, step.formula, ""];    	 	
		var onError = function (data) {alert('Error calling ready service');};
    ...
    this.removeTopStep = function (source) {
        var parent = source.parents('div.exercise-step-added'),
            allExerciseSteps = $('#exercise-steps div.exercise-step-added'),
            index = allExerciseSteps.index(parent),
			step = this.exercise.getCurrentStep(),
			state = [this.exercise.type, step.strategyStatus, step.formula, ""];   
    ...
    this.removeBottomStep = function (source) {
        ...
        var parent = source.parents('div.exercise-step-added-bottom'),
            allExerciseSteps = $('#exercise-steps div.exercise-step-added-bottom'),
